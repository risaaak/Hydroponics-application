...
SELECT DATE,AMOUNT FROM WEEKLYINCOME
INSERT INTO WEEKLYFINANCETEMP(DATE,CASH_OUTFLOW)
SELECT DATE,AMOUNT FROM WEEKLYEXPENSE
DROP TABLE IF EXISTS WEEKLYFINANCE
SELECT MAX(DATE) AS DATE, SUM(CASH_OUTFLOW) AS CASH_OUTFLOW,SUM(CASH_INFLOW) AS CASH_INFLOW, SUM(ISNULL (CASH_OUTFLOW,0) + ISNULL(CASH_INFLOW,0)) AS EARNINGS--FROM WEEKLYFINANCETEMP --GROUP BY DATEPART(ISO_WEEK,DATE)
INTO WEEKLYFINANCE FROM WEEKLYFINANCETEMP GROUP BY DATEPART(ISO_WEEK,DATE)
GO

DROP PROCEDURE INSERTINTOWEEKLYFINANCE
EXEC INSERTINTOWEEKLYFINANCE
