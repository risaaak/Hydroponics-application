...
CREATE PROCEDURE INSERTOUTFLOWINTOWEEKLYFINANCE
AS
	BEGIN
		IF EXISTS(SELECT * FROM WEEKLYFINANCE WHERE DATE = 332023)
		BEGIN
...
--INSERT TO WEEKLYFINANCE
SELECT MAX(DATE), SUM(CASH_OUTFLOW),SUM(CASH_INFLOW) FROM WEEKLYFINANCETEMP GROUP BY DATEPART(ISO_WEEK,DATE)
SELECT * FROM WEEKLYFINANCETEMP